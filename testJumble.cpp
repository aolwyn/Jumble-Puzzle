...
cout << "test the erorr messages. should be 4 in total:"<<endl;

    cout << "test invalid difficulty x2:"<<endl;
//test the invalid input
try{
    JumblePuzzle jp27("ABC", "chill");
}catch(BadJumbleException& e){
    cerr << e.what() <<endl;
}
//test uppercase wrong difficulty
try{
    JumblePuzzle jp32("ABC", "HARD");
}catch(BadJumbleException& e){
    cerr<< e.what()<<endl;
}
    cout << "test less than 3 char input:"<<endl;

//test less than 3 char input
try{
    JumblePuzzle jp38("ab", "easy");
}catch(BadJumbleException& e){
    cerr << e.what()<<endl;
}

    cout << "test more than 10 char input:"<<endl;

//test more than 10 input.
try{
    JumblePuzzle jp40("qwertyuioplkjhgfd", "easy");
}catch(BadJumbleException& e){
    cerr <<e.what()<<endl;
}

    //check the sizes.
    cout<< "test correct size. If test fails, getSize() also fails:"<<endl;
    JumblePuzzle jp07("HELLO", "easy");
    JumblePuzzle jp08("HELLO", "medium");
    JumblePuzzle jp09("HELLO", "hard");

    //should be in correct multiples of 5 (5*2, 5*3, 5*4 because hello is 5 characters long)
    if(jp07.getSize() == 10 && jp08.getSize() == 15 && jp09.getSize() == 20){
        cout<< "Correct sizes."<<endl;
    }else{
...
cout<< "testing the row and column get methods:"<<endl;
    string word = "WORD";
    JumblePuzzle jp12(word, "easy");
    charArrayPtr* jumble = jp12.getJumble();

    char direction = jp12.getDirection();
    int row = jp12.getRowPos();
    int column = jp12.getColPos();

    if(jumble[row][column] == word.at(0)){
        cout<<"row and column test passed."<<endl;
    }else{
        cout<<"row and column test failed."<<endl;
    }

    cout<< "testing all together, withd direction (get):"<<endl;
    cout<<"first letter should be in row: "<<row<<" column: "<<column<<" and follow the direction of: "<<direction;
    cout<<"Please check the jumble below to verify the correct positions."<<endl;
    showJumble(jumble, jp12.getSize());

    //all other errors that could appear will be checked in the code provided. No additional tests should be needed.
    //END OF MY TESTS. **********************************
